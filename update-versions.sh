...
json=$(curl -fsSL --header "Authorization: Bearer ${GITHUB_TOKEN}" "https://api.github.com/repos/whisparr/whisparr/actions/runs" | jq -re '[.workflow_runs[] | select(.name == "Build") | select(.conclusion == "success") | select(.event == "pull_request") | select(.actor.login != "dependabot[bot]")| .][0]')
