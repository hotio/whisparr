...
json=$(curl -fsSL --header "Authorization: Bearer ${GITHUB_TOKEN}" "https://api.github.com/repos/Whisparr/Whisparr-Eros/releases" | jq -re 'map(select(.prerelease == true)) | first')
